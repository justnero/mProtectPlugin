...
public class DeleteCommand extends BasicCommand {
    ...
    static {
        requiredPermissions = new String[]{"user","delete"};
    }
    ...
    public boolean execute(final Player player, String[] args) {
        ...
        if(args.length == 1) {
            playerToDelete = args[0].toLowerCase();
        } else if(args.length == 2) {
            protectName = args[0].toLowerCase();
            playerToDelete = args[1].toLowerCase();
        } else {
            player.sendMessage(langGet("wrongArguments"));
            player.sendMessage(langGet("commandDelete"));
            return true;
        }
        ...
        boolean isFakePlayer = playerToDelete.startsWith("[") && playerToDelete.endsWith("]");
        ProtectedRegion region = mgr.getRegion(regionName);
        DefaultDomain members = isFakePlayer ? region.getOwners() : region.getMembers();
        if(!members.contains(playerToDelete)) {
            player.sendMessage(langGet("protectNotMember"));
            return true;
        }
        members.removePlayer(playerToDelete);
        if(isFakePlayer) {
        ...
        try {
            ...
            player.sendMessage(langGet("protectDeleted"));
